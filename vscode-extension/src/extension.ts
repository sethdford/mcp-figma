...
function updateStatusBarItem(status: 'connected' | 'disconnected' | 'starting' | 'error') {
...
async function setupMcpServer() {
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            `✅ MCP server configured for ${result.label}. Restart your AI assistant to use the new configuration.`,
            'Open Documentation'
...
async function configureMcpServer(assistant: string) {
    const mcpConfig = {
        "TalkToFigma": {
            "command": "npx",
...
async function showManualConfiguration(mcpConfig: any) {
    ...
    vscode.window.showInformationMessage(
        'Copy this configuration to your AI assistant\'s MCP configuration file',
        'Copy to Clipboard'
    ).then((selection: string | undefined) => {
...
async function startWebSocketServer() {
    ...
    try {
        ...
        webSocketProcess = spawn('node', [socketScriptPath], {
...
${isRunning ? '✨ Ready to connect with Figma!' : '⚠️ Start the WebSocket server to begin'}`;
    ...
    const selection = await vscode.window.showInformationMessage(
        status,
        ...(isRunning ? ['Test Connection', 'Stop Server'] : ['Start Server']),
        'Open Settings'
    );
...
async function checkMcpConfiguration() {
    ...
    if (!mcpConfig || !(mcpConfig as any).servers?.TalkToFigma) {
        // Show notification to set up MCP
        const selection = await vscode.window.showInformationMessage(
            '🎨 MCP Figma is not configured. Set it up now to start using AI-powered Figma automation!',
            'Setup Now',
            'Don\'t Show Again'
        );
        ...
        if (!element) {
            ...
            return Promise.resolve([
                new TreeItem(
                    ...
                    'status',
                    isRunning ? 'mcpFigma.stopWebSocketServer' : 'mcpFigma.startWebSocketServer'
                ),
